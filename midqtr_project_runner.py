...
def midqtr_tests():
    ...
    #看value一不一样#
    >>> rgb_1.get_pixels() == rgb_1.pixels
    True

    #看是不是deep copy#
    >>> rgb_1.get_pixels() is rgb_1.pixels
    False
    >>> isinstance(rgb_1.copy(), RGBImage)
    True

    >>> rgb_1.copy().pixels == rgb_1.pixels
    True
    >>> rgb_1.copy().num_rows == rgb_1.num_rows
    True
    >>> rgb_1.copy().num_cols== rgb_1.num_cols
    True
    >>> rgb_1.copy() is rgb_1
    False
    ...
    >>> rgb_1.set_pixel(4, 9, (222, 223, 224))
    >>> rgb_1.get_pixel(4, 9)
    (222, 223, 224)

    #还原到原来的样子#
    >>> rgb_1.set_pixel(4, 9, (223, 127, 202))
    >>> rgb_1.get_pixel(4, 9)
    (223, 127, 202)

    #测 new_color 里数字有负的情况#
    >>> rgb_1.set_pixel(4, 9, (-222, 223, 224))
    >>> rgb_1.get_pixel(4, 9)
    (223, 223, 224)
    ...
    #测 tint 的 test#
    >>> tint_dsc20_img = IP.tint(dsc20_img, color=(0,0,100))
    >>> img_save("img/out/dsc20_tint.png", tint_dsc20_img)

    #测 clear_channel 的 test1#
    >>> clear_dsc20_img1 = IP.clear_channel(dsc20_img, 0)
    >>> img_save("img/out/dsc20_clear1.png", clear_dsc20_img1)

    #测 clear_channel 的 test2#
    >>> clear_dsc20_img2 = IP.clear_channel(dsc20_img, 2)
    >>> img_save("img/out/dsc20_clear2.png", clear_dsc20_img2)

    #测 crop 的 test1#
    >>> crop_dsc20_img1 = IP.crop(dsc20_img,50,75,(75,50))
    >>> img_save("img/out/dsc20_crop1.png", crop_dsc20_img1)

    #测 crop 的 test2#
    >>> crop_dsc20_img2 = IP.crop(dsc20_img,100,50,(100,150))
    >>> img_save("img/out/dsc20_crop2.png", crop_dsc20_img2)

    #测 rotate 的 test#
    >>> rotate_dsc20_img = IP.rotate_180(dsc20_img)
    >>> img_save("img/out/dsc20_rotate.png",rotate_dsc20_img)

    #part3 test#
    >>> train = []
    >>> train.extend((RGBImage(create_random_pixels(0, 75, 300, 300)), "low") for _ in range(20))
    >>> train.extend((RGBImage(create_random_pixels(180, 255, 300, 300)), "high") for _ in range(20))
    >>> knn = ImageKNNClassifier(5)
    >>> knn.fit(train)
    >>> knn.predict(RGBImage(create_random_pixels(0, 75, 300, 300)))
    'low'
    >>> knn.predict(RGBImage(create_random_pixels(75, 180, 300, 300)))
    'low'
    >>> knn.predict(RGBImage(create_random_pixels(180, 255, 300, 300)))
    'high'
    """
    return
